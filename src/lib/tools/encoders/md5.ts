...
function md5(input: string): string {
  ...
  for (const val of [a, b, c, d]) {
    for (let j = 0; j < 4; j++) {
      result += hex.charAt((val >> (j * 8 + 4)) & 0x0f) + hex.charAt((val >> (j * 8)) & 0x0f);
    }
  }
  return result;
}

export const md5Generator: ToolDefinition = {
  id: "md5-generator",
  name: "MD5 Hash Generator",
  category: "encoders",
  route: "/encoders/md5",
  language: "text",
  description: "Generate MD5 hash digests from any text input. Note: MD5 is not cryptographically secure.",
  keywords: ["md5", "hash", "digest", "checksum", "fingerprint", "message digest"],
  mode: "transform",
  ...
  execute: async (input) => {
    try {
      ...
      return {
        success: true,
        output: `Input: ${input}\n\nMD5: ${hash}\n\nNote: MD5 is not recommended for security purposes.`,
      };
    } catch (e) {
      const msg = e instanceof Error ? e.message : "MD5 generation failed";
